 ...
 */
function calculateStats(patrols: Patrol[], participants: Map<string, Participant>): PatrolStats {
  if (patrols.length === 0) {
    return {
  ...
  for (const patrol of patrols) {
    const divisions = patrol.members.map((id) => participants.get(id)?.division).filter(Boolean);
  ...
  for (const patrol of patrols) {
    const genders = patrol.members.map((id) => participants.get(id)?.gender).filter(Boolean);
