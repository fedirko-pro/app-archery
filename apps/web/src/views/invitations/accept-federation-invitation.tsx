...
const AcceptFederationInvitation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const accept = async () => {
      try {
        await apiService.acceptFederationInvitation(id);
      } catch (err) {
        setError(
    ...
    return (
      <Box sx={{ p: 3, maxWidth: 500, mx: 'auto', textAlign: 'center' }}>
        ...
        <Typography color="text.secondary" sx={{ mb: 3 }}>
          {t(
            'invitations.loginToAcceptFederation',
            'Please sign in to accept this federation invitation.',
          )}
        </Typography>
        <Button
          variant="contained"
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 500, mx: 'auto', textAlign: 'center' }}>
      <Alert severity="success" sx={{ mb: 2 }}>
        {t('invitations.federationAccepted', 'Your club has joined the federation!')}
      </Alert>
...
export default AcceptFederationInvitation;
